...
interface SalonMediaUploaderProps {
  salonId: string;
  maxFiles?: number;
}
...
export default function SalonMediaUploader({ salonId, maxFiles = 50 }: SalonMediaUploaderProps) {
  ...
  }, [salonId, mediaList.length, maxFiles, uploadMutation, toast]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold">Salon Media Gallery</h2>
          <p className="text-gray-600">
            {mediaList.length} of {maxFiles} files uploaded
          </p>
        </div>
        <Dialog open={showUploadDialog} onOpenChange={setShowUploadDialog}>
          <DialogTrigger asChild>
            <Button disabled={mediaList.length >= maxFiles}>
          ...
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Upload up to {maxFiles - mediaList.length} more files. Supported: Images (JPG, PNG, WebP) and Videos (MP4, WebM, MOV). Max size: 100MB per file.
              </DialogDescription>
            </DialogHeader>
      ...
      <div className="w-full bg-gray-200 rounded-full h-2">
        <div
          ...
          style={{ width: `${(mediaList.length / maxFiles) * 100}%` }}
        />
      </div>
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        {viewingMedia.description && (
                          <p className="text-sm opacity-80 mt-1" data-testid="text-viewer-description">
                            {viewingMedia.description}
                          </p>
                        )}
